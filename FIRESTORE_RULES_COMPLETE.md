# Complete Firestore Security Rules - Updated with Delete Permissions

## Overview

This document contains the complete Firestore security rules with delete permissions for conversations and messages enabled.
...
4. Copy and paste the complete rules below
...
- **Result:** Users can delete messages where they are sender or receiver
...
- **Result:** Users can delete conversations where they are a participant

## Quick Update Steps:

1. Go to Firebase Console → Firestore Database → Rules
2. Find the `messages` collection rules
3. Change `allow delete: if false;` to:
   ```javascript
   allow delete: if isAuthenticated() && 
                    (resource.data.senderId == request.auth.uid ||
                     resource.data.receiverId == request.auth.uid);
...
5. Change `allow delete: if false;` to:
   ```javascript
   allow delete: if isAuthenticated() && 
                    (resource.data.participant1Id == request.auth.uid ||
                     resource.data.participant2Id == request.auth.uid);
...
2. Try deleting a conversation in the app
3. Check browser console for any errors
4. Verify the conversation is deleted from Firestore
5. Verify messages are deleted from Firestore
...
4. **Verify authentication**: Make sure user is authenticated (`auth.currentUser` is not null)
